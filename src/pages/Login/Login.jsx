...
function Login() {
    ...
    const handleLogin = async (e) => {
        e.preventDefault()
        try {                                   //macin bridge100 ipsi, geçici olarak en0 yazacağım
            let response = await axios.post(`http://172.20.10.2:${config.apiPort}/api/login`, { username, password });
            console.log("responsee", response);
            
            if (response.data.success) {
               response = await axios.get(`http://172.20.10.2:${config.apiPort}/api/user/${response.data.username}`);
